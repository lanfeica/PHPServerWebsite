...
class employeeDAO extends abstractDAO {
    ...
    public function getEmployee($employeeId){
        ...
        if($result->num_rows == 1){
            ...
            $employee = new employee($temp['id'],$temp['name'], $temp['address'], $temp['salary'], $temp['birthdate'], $temp['filename']);
            $result->free();
            return $employee;
        }
        $result->free();
    ...
    public function getEmployees(){
        ...
        if($result->num_rows >= 1){
            while($row = $result->fetch_assoc()){
                ...
                $employee = new Employee($row['id'], $row['name'], $row['address'], $row['salary'], $row['birthdate'], $row['filename']);
                $employees[] = $employee;
            }
            $result->free();
            return $employees;
    ...
    public function addEmployee($employee){
        ...
        if(!$this->mysqli->connect_errno){
			...
			$query = 'INSERT INTO employees (name, address, salary, birthdate, filename) VALUES (?,?,?,?,?)';
			$stmt = $this->mysqli->prepare($query);
            if($stmt){
    ...
    public function updateEmployee($employee){
        ...
        if(!$this->mysqli->connect_errno){
            ...
            //a mysqli_stmt object. It takes a parameterized 
            //query as a parameter.
            $query = "UPDATE employees SET name=?, address=?, salary=?, birthdate=?, filename=? WHERE id=?";
            $stmt = $this->mysqli->prepare($query);
            if($stmt){
                    $id = $employee->getId();
                    $name = $employee->getName();
